...
def rollout_model(model, num_demos, task_name='phone_on_base', max_execution_steps=30,
    ...
    ####################################################################################################################
    track_buffer = None
    if model.config.get('enable_track_nodes', False):
        track_buffer = TrackBuffer(
            track_n_max=model.config.get('track_n_max', 2),
            track_history_len=model.config.get('track_history_len', 16),
            track_points_per_obj=model.config.get('track_points_per_obj', 5),
    ...
    for i in pbar:
        ...
        for k in range(max_execution_steps):
            ...
            if track_buffer is not None:
                track_buffer.update(get_point_cloud(curr_obs), T_w_e)
